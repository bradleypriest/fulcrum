var StoryView = FormView.extend({
  ...
  render: function() {
    if(this.model.get('editing') === true) {
      ...
      if (!this.model.isNew()) {
        $(div).append(
          this.make("img", {'class': "collapse", 'src': "/assets/collapse.png"})
        );
      }
  ...
  disableForm: function() {
    ...
    $(this.el).find('img.collapse,img.expand').attr('src', "/assets/throbber.gif");
  },

  enableForm: function() {
    $(this.el).find('img.collapse').attr('src', "/assets/collapse.png");
  }
});
